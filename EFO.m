...
%**************************************************************************************************
function [best_position]= EFO(N_var,N_emp,Max_gen,minval,maxval,R_rate,Ps_rate,P_field,N_field,problemIndex)
phi = (1 + sqrt(5))/2;%golden ratio
%initializatin
em_pop = minval + (maxval - minval) .* rand(N_emp, N_var);
fit= cec17_func(em_pop(1:N_emp,1:N_var)',problemIndex);
...
while (generation <= Max_gen)
    ...
    new_emp(N_var+1) = cec17_func(new_emp(1:N_var)',problemIndex);
...
best_position=em_pop(1,1:N_var);
% em_pop=em_pop(:,1:N_var);
end